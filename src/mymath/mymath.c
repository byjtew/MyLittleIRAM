...
void IRAM_computeNewInputVector(const vector_t *restrict input,
                                const matrix_t *restrict eigen_vectors)
{
...
eigenData_t IRAM(const matrix_t *restrict A, const size_t n_eigen, const size_t max_iter,
                 const double max_error)
{
  ...
  while (1)
  {
    ...
    IRAM_computeEigenSubspace(&buffer.H, &eigen.eigen_val_r, &eigen.eigen_val_i, &buffer.Z);
    ...
    eigenQuickSort(&eigen, &buffer.buffer_eigen_sort, 0, eigen.eigen_val_r.n - 1);
    ...
    for (size_t i = 0; i < p; i++)
    {
      ...
      // matrix_print_colmajor(&H);
      // Compute Qj* x H
      cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, Qj.row - 1, buffer.H.column,
    ...
    {
      buffer.f.data[i] = buffer.V.data[MAT_GET_CMAJOR(buffer.V, buffer.V.column - 1, i)] * kBeta +
    ...
    for (size_t y = k; y < buffer.H.row; y++)
    {
      ...
      {
        buffer.H.data[MAT_GET_CMAJOR(buffer.H, x, y)] = .0;
      }
    }
